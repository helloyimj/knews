...
//파일다운로드 로딩바 생성_에러시 프라이버시 popup
function fn_filedown_progress_privacy(url, filename) {
...
// 설명 : 파일 차단 안내 페이지를 830x900 팝업으로 오픈
function fn_openBlockedFilePopup(pUrl){
	var width = 1000, height = 861;
	...
	window.open(pUrl, 'blockedFilePop', windowFeatures);
	
//	$('body').css("overflow-y","hidden");
//	$('<iframe id="iframeBlockedFilePop" src="'+pUrl+'" class="pop_iframe on" height="100%" scrolling="no" frameborder="0" title="첨부파일 차단 안내 팝업"/>').appendTo($("body #container"));
