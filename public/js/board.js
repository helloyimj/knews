...
// 		$(".writeIp").text(ip());
	...
	$(document).on('click', '.aiRecommArea .btnC_s', function(){
		$(document).find('body').css("overflow-y","hidden");
		$('<iframe id="iframePop" class="pop_iframe on" src="/cms/news/actionKotraAiRecommPop.do?gbn='+$(this).data('gbn')+'&pSiteNo=3&pMenuNo=180&pContSn='+$('#sendForm [name=pDataId]').val()+'&pa1='+$('#sendForm [name=apiNat]').val()+'&pa2='+$('#sendForm [name=apiHscd]').val()+'" title="[팝업] 드림AI추천" frameborder="0" />').appendTo($(document).find("body #container"));
	});
});
 ...
 function fn_goPromisingBuyerSearch(){
    ...
    fn_comm_ajax({
    	...
    	success: function(data){
			...
			if(data != null){
				if(data.RESULT == 'SUCCESS'){
				    var url = 'https://www.kotra.or.kr/bigdata/ext/getBlEntpList?hscds='+hscds+'&countryCd='+countryCd+'&st=';
 ...
 ************************************************************************/
 function  fn_filePreview(obj){	
     ...
     dataObj.pGbn = 'n01';
     dataObj.nttSn = $("#sendForm [name=pNttSn]").val();moiu
