...
//	})
	...
	$(document).on('click', '.listViewType button', function(){
		...
		location.href = "/kotranews/cms/com/index.do?MENU_ID=80&CONTENTS_NO=2&viewType=" + viewType + "&"+$("#sendForm").serialize();
	});
		...
		for(var idx=0; idx<data.length; idx++){
			...
			if(null != attachList && '' != attachList && 0 < attachList.length){
				...
				for(var idx2=0; idx2<attachList.length; idx2++){
					...
					obj += '<img src="https://dream.kotra.or.kr/kotranews/type/common/img/common/file_'+attachList[idx2].ATFILE_EXT_NAME+'.png" alt="'+attachList[idx2].NTT_ATFILE_IMG_MVP_DC_CN+'" class="ico_file_'+attachList[idx2].ATFILE_EXT_NAME+'" onError="this.src=\'/type/common/img/common/file_etc.png\'"/>';
					obj += '</a>&nbsp;';
				}
...
// 		window.open("/kotranews/cms/news/actionKotraBoardDownDetail.do?MENU_ID=80&pNttSn=" + nttSn + "&bbsSn=" + bbsSn + "&type=" + type);
		var src = "/kotranews/cms/news/actionKotraBoardDownDetail.do?MENU_ID=80&pNttSn=" + nttSn + "&bbsSn=" + bbsSn;
	...
	$('.pdfFrame').load(function(){
		...
		if(type == 'pdf'){
			...
			var content =	"<!doctype html>" 
						...
						+	"<head>"
						+	"<meta charset=\"utf-8\">"
						+	"<link rel='stylesheet' type='text/css' href='"+hostName+"/type/common/css/board.css' />"
						+	"<link rel='stylesheet' type='text/css' href='"+hostName+"/type/common/css/common.css' />"
						+	"<title>pdf</title>"
						+	"</head>"
						+	"<body class='typeSub'>"
